import { Body, Controller, Delete, Get, HttpException, Param, Post, Put, Query, Req, Res } from '@nestjs/common';
...
export class NilaiController {
    ...
    async get(@Query() query: QueryNilaiDto, @Res() res: Response) {
        ...
        try {
            const {
                ...
                limits,
                totalPages,
            } = await this.nilaiService.get(siswa_id, status_seleksi, status_verifikasi, filterDate, kode_pendaftaran, page, limit);
    ...
    @Get('generate-pdf-kelulusan')
    async pdfKelulusan(@Req() req: Request, @Res() res: Response) {
        try {
            const data = await this.nilaiService.generatePdfKelulusan();
    ...
    async generateExcel(@Res() res: Response) {
        try {
            const data = await this.nilaiService.generateExcel();
    ...
    async generatePdf(@Param('id') payload: string, @Res() res: Response) {
        try {
            const data = await this.nilaiService.generatepdf(payload);
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', 'attachment; filename=HASIL SELEKSI.pdf');
            // res.setHeader('Content-Disposition', 'inline; filename=HASIL SELEKSI.pdf');
    ...
    async getId(@Param('id') id: string, @Res() res: Response) {
        try {
            const data = await this.nilaiService.getId(id)
    ...
    async create(@Body() payload: CreateNilaiDTO, @Res() res: Response) {
        try {
            const data = await this.nilaiService.create(payload);
    ...
    async update(@Param('id') id: string, @Body() payload: UpdateNilaiDTO, @Res() res: Response) {
        try {
            const data = await this.nilaiService.update(id, payload)
